...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Generate test data for Snowflake signup"
    )
    parser.add_argument(
        "-c",
        "--count",
        type=int,
        default=3,
        help="Number of test data entries to generate",
    )
    parser.add_argument(
        "-o", "--output", default="test_data.json", help="Output JSON file path"
    )
    parser.add_argument(
        "-p", "--print", action="store_true", help="Print the generated data to console"
    )
